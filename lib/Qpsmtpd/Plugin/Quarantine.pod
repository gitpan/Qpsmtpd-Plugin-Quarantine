...
Qpsmtpd::Plugin::Quarantine implements and outbound mail filter.
...
This is a L<htpasswd>-style password file that controls access to the admin
web page.  Create it with C<htpasswd -c /etc/qpsmtpd/quarantine.access adminuser>.
...
command will dispay what's in it.  Messages in quarantine are not in
the mail queue.
